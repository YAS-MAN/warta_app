                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: const Text("PENGURUS RW", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10)),
                          ),
                        ],
                      ),
    ...
    return Consumer<AuthViewModel>(
      builder: (context, authVM, _) {
        ...
        return StreamBuilder<List<ReportModel>>(
          stream: rw.isNotEmpty ? _reportService.streamReportsForRw(rw: rw) : Stream.value([]),
          builder: (context, snapshot) {
            final count = snapshot.data?.length ?? 0;
            final message = count > 0
                ? "Terdapat $count laporan dari RT yang membutuhkan tindak lanjut Anda."
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text("Laporan Terbaru dari RT", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: textDark)),
              TextButton(
          ...
          Consumer<AuthViewModel>(
            builder: (context, authVM, _) {
              final rw = authVM.currentUser?.rw ?? '';
              if (rw.isEmpty) return _buildEmptyState();
              return StreamBuilder<List<ReportModel>>(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text("RT ${report.reporterRt} · ${report.reporterName}", style: const TextStyle(fontSize: 12, color: Colors.grey)),
                ],
              ),
            ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 12),
          Text("Belum ada laporan masuk dari RT.", style: TextStyle(color: Colors.grey.withValues(alpha: 0.5), fontSize: 14)),
        ],
      ),
    );
  }

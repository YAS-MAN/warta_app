...
import 'lurah_rw_list_view.dart';

class LurahProfilView extends StatefulWidget {
  const LurahProfilView({super.key});

  @override
  State<LurahProfilView> createState() => _LurahProfilViewState();
}

class _LurahProfilViewState extends State<LurahProfilView> {
  ...
  Future<void> _showSignatureUploadSheet(AuthViewModel authVM) async {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Pilih sumber gambar tanda tangan Lurah", style: TextStyle(fontSize: 12, color: textGray)),
            const SizedBox(height: 20),
            Row(
              children: [
                Expanded(child: _buildPhotoBtn(icon: Icons.camera_alt_rounded, label: "Kamera", onTap: () => Navigator.pop(ctx, "camera"))),
    ...
    try {
      final uploadedUrl = await _cloudinaryService.uploadImageXFile(imageFile, folder: 'lurah_signatures');
      if (uploadedUrl == null || uploadedUrl.isEmpty) throw Exception('Upload gagal.');

      await FirebaseFirestore.instance.collection('users').doc(user.uid).update({'lurahSignatureUrl': uploadedUrl});
  ...
  Widget _buildIdentityCard(AuthViewModel authVM) {
    ...
    final signatureUrl = user?.lurahSignatureUrl ?? '';
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(kelurahan.isNotEmpty ? kelurahan.toUpperCase() : "KELURAHAN", style: const TextStyle(color: Colors.white, fontSize: 9)),
                ),
              ],
            ),
            const SizedBox(height: 18),
            const Text("NAMA LURAH", style: TextStyle(color: Colors.white70, fontSize: 9)),
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: const Text("LURAH", style: TextStyle(color: goldColor, fontWeight: FontWeight.bold, fontSize: 10)),
                    ),
                  ],
                ),
                Container(
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              const Text("Profil Eksekutif", style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            _buildMenuItem(Icons.drive_file_rename_outline_rounded, "Tanda Tangan Digital", subtitle: "Atur tanda tangan resmi Lurah", onTap: () => _showSignatureUploadSheet(authVM)),
                            const Divider(height: 1, color: borderColor),
                            _buildMenuItem(
                              Icons.people_alt_outlined, 
                              "Daftar Penduduk Kelurahan", 
                              subtitle: "Kelola peran pengurus (Angkat/Berhentikan RT & RW)", 
                              onTap: () {
                                Navigator.push(context, MaterialPageRoute(builder: (_) => LurahRwListView(kelurahan: authVM.currentUser?.kelurahan ?? '')));
                              },
                            ),
                            const Divider(height: 1, color: borderColor),

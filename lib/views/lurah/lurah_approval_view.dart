...
class LurahApprovalView extends StatelessWidget {
  final int initialIndex;
  const LurahApprovalView({super.key, this.initialIndex = 0});
...
// ═══════════════════════════════════════════════════════════════════
// TAB 1: LAPORAN MASUK TINGKAT KELURAHAN
// ═══════════════════════════════════════════════════════════════════
class _TabLaporanMasuk extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<ReportModel>>(
      stream: reportService.streamReportsForLurah(),
...
// ═══════════════════════════════════════════════════════════════════
// TAB 2: SURAT KELURAHAN (PROSES AKHIR LURAH)
// ═══════════════════════════════════════════════════════════════════
class _TabSuratKelurahan extends StatefulWidget {
  const _TabSuratKelurahan();

  @override
  State<_TabSuratKelurahan> createState() => _TabSuratKelurahanState();
}

class _TabSuratKelurahanState extends State<_TabSuratKelurahan> {
  final SuratService _suratService = SuratService();

  void _showSuratPreview(BuildContext context, SuratSubmissionModel submission) {
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        const Text("Preview Surat Kelurahan", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1F2937))),
                        ...
                        children: [
                          Container(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                _previewRow("Jenis Surat", submission.jenisSurat),
                                const SizedBox(height: 8),
                                _previewRow("Nama Pemohon", submission.nama),
                                const SizedBox(height: 8),
                                _previewRow("NIK", submission.nik),
                                const SizedBox(height: 8),
                                _previewRow("RT/RW", "${submission.rt}/${submission.rw}"),
                                const SizedBox(height: 8),
                          ...
                          const Text("Isi Surat", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1F2937))),
                          const SizedBox(height: 12),
                          Container(
                            width: double.infinity,
                            padding: const EdgeInsets.all(16),
                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE5E7EB))),
                            child: (submission.customBody != null && submission.customBody!.isNotEmpty)
                                ? Text(submission.customBody!, style: const TextStyle(fontSize: 13, height: 1.7, color: Color(0xFF374151)))
                                : FutureBuilder<SuratModel?>(
                                    future: _suratService.getSuratByTitle(submission.jenisSurat),
                                    builder: (context, snapshot) {
                                      final fallbackText = snapshot.data?.templateKonten ??
                                          "Menerangkan bahwa surat kelurahan ini diterbitkan secara resmi sebagai kelengkapan administrasi kependudukan warga kelurahan setempat.";
                                      return Text(fallbackText, style: const TextStyle(fontSize: 13, height: 1.7, color: Color(0xFF374151)));
                                    },
                                  ),
                          ),
                        ],
  ...
  Widget _previewRow(String label, String value) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(width: 110, child: Text(label, style: const TextStyle(color: Color(0xFF64748B), fontSize: 12, fontWeight: FontWeight.w600))),
        const Text(": ", style: TextStyle(color: Color(0xFF64748B), fontSize: 12)),
        Expanded(child: Text(value, style: const TextStyle(color: Color(0xFF1E293B), fontSize: 12, fontWeight: FontWeight.w600))),
      ],
    );
  }
    ...
    return Consumer<AuthViewModel>(
      builder: (context, authVM, _) {
        ...
        if (kelurahan.isEmpty) {
          return const Center(child: Text("Data wilayah Kelurahan belum lengkap."));
        }

        return StreamBuilder<List<SuratSubmissionModel>>(
          stream: _suratService.streamSubmissionsForLurah(kelurahan: kelurahan),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(child: CircularProgressIndicator(color: Color(0xFF8B0000)));
            }
                            ...
                            children: [
                              Expanded(
                                child: ElevatedButton(
                                  onPressed: () async {
                                    await _suratService.updateSubmissionStatus(submissionId: sub.id, newStatus: 'BERHASIL', actedByUid: user!.uid);
                                    if (!context.mounted) return;
                                    TopNotification.show(context: context, message: "Surat resmi disahkan dan diterbitkan.", isSuccess: true);
                                  },
                                  ...
                                  child: const Text("Sahkan Surat", style: TextStyle(color: Colors.white)),
                                ),
                              ),
